...
export default function SeedBlogsPage() {
  ...
  const handleSeed = async () => {
    ...
    try {
      const response = await seedBlogs();
      setResult(response);

      if (response.success) {
        toast.success(
          `Đã seed ${response.summary?.created || 0} blog thành công`
        );
      } else {
        toast.error(response.error || "Có lỗi xảy ra khi seed blogs");
      }
    } catch (error) {
  ...
  return (
    <div className="container mx-auto py-8 max-w-4xl">
      <Card className="p-6">
        <h1 className="text-2xl font-bold mb-4">Seed Blogs từ Frontend</h1>
        <p className="text-muted-foreground mb-4">
          Chức năng này sẽ thêm các bài blog mẫu từ frontend vào Strapi backend.
          Các blog đã tồn tại sẽ được bỏ qua.
        </p>
          ...
          <ol className="list-decimal list-inside space-y-1 text-sm text-blue-800 dark:text-blue-200">
            ...
            <li>Tạo API Token trong Strapi Admin Panel:
              <ul className="list-disc list-inside ml-4 mt-1">
                ...
                <li><strong>Chọn Type: <span className="text-red-600 dark:text-red-400">Full access</span></strong> (khuyến nghị)</li>
...
STRAPI_API_TOKEN=your-token-here
            ...
            <li className="text-xs text-yellow-700 dark:text-yellow-300 mt-1">
              ⚠️ Lưu ý: Blog posts sẽ được tạo nhưng có thể thiếu coverImage (required field).
              Bạn cần upload coverImage sau trong Strapi Admin Panel.
            </li>
          </ol>
        </div>

        <Button onClick={handleSeed} disabled={loading}>
          {loading ? "Đang xử lý..." : "Seed Blogs"}
        </Button>
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-1 text-sm max-h-60 overflow-y-auto">
                    {result.results.map((r: any, idx: number) => (
                      <p key={idx}>
                        {r.status === "created" ? "✅" : "⚠️"} {r.blog} ({r.status})
                      </p>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-2 text-sm">
                    {result.errors.map((e: any, idx: number) => (
                      <div key={idx} className="p-2 bg-destructive/10 rounded border border-destructive/20">
                        <p className="text-destructive font-semibold">❌ {e.blog}:</p>
